...
double q[9], r[2], c[4], cold[4], vold, v;
double rho, rhold, cpc, cpcold, t1, t2, t4, qrxn, qext, qc;
double dxsav, *xp, **yp;
int kount, kmax, nok, nbad;

void concentrations(double x, double y[], double dydx[]){
	dydx[1] = - r[0] - r[1] + (1/v)*((cold[0] - cold[1])*q[1] - cold[1]*q[6]);
	dydx[2] = r[0] - (cold[2]/v)*(q[1] + q[6]);
	dydx[3] = r[1] - (cold[3]/v)*(q[1] + q[6]);
}

void reac_temp(double x, double y[], double dydx[]){
	dydx[1] = ((qrxn + qext - qc)/(rhold*cpcold)) + q[1]*(t1 - t2) + q[6]*(t4 - t2);
}
...
int procQuimRK(int argc, char *argv[]){
	...
	//Temperature vector
	double t[5], c[4], cc0, dn, dt, ktb1;
	double v0;//, vold, rhold, cpc, cpcold;
	...
	double *vca, *vt2;
	double eps, stepsize, minstepsize, kmax, dxsav;
	...
	xp = dvector(1,200);
	yp = dmatrix(1,3,1,200);
	vca = dvector(1, 3);
	vt2 = dvector(1, 2);

	//Initial values for concentrations
	c[0] = cold[0] = 20.0;	//Feed concentration
	c[1] = cold[1] = 2.850;	//Product A concentration
	c[2] = cold[2] = 17.114;	//Product B concentration
	c[3] = cold[3] = 0.0226;	//Product C concentration
	cc0 = c[3];
	...
	while(simTime < maxTime) {
		...
		odeint(vca, 3, 0.0, dt, eps, stepsize, 0.0, &nok, &nbad, (void*)concentrations, rkqs);
		...
		//odeint
		t1 = t[1]; t2 = t[2]; t4 = t[4];
		vt2[1] = t[2];
		odeint(vt2, 1, 0.0, dt, eps, stepsize, 0.0, &nok, &nbad, (void*)reac_temp, rkqs);
		t2_old = t[2];
		t[2] = vt2[1];
		//fprintf(stdout, "%lf\n", t[2]);
	...
	fclose(matlab34);
	fclose(matlab14);
	free_dvector(vca, 1, 3);
	free_dvector(vt2, 1, 2);
	free_dvector(xp, 1, 200);
	free_dmatrix(yp, 1, 3, 1, 200);
